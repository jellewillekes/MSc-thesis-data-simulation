...
def plot_target_distribution(ax_target):
    """Plot the distribution of the target variable."""
    sns.histplot(dataset, bins=50, x=TARGET_COL, color="purple", alpha=0.5, ax=ax_target, kde=False, label=TARGET_COL)
    ax_target.set_title(f'Distribution of {TARGET_COL}')
    ax_target.legend()
    ax_target.set_xlim(dataset[TARGET_COL].min(), 80)
    ax_target.set_xlabel(f'Value of {TARGET_COL}', fontsize=8)
    ax_target.set_ylabel('Frequency', fontsize=8)
...
fig.subplots_adjust(left=0.05, right=0.95, top=0.95, bottom=0.05, wspace=0.4, hspace=0.5)
...
for unused_subplot_index in range(index + 2, num_rows * NUM_SUBPLOTS_PER_ROW):
